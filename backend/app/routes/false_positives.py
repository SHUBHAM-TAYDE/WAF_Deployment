...
async def mark_log_as_false_positive(
    ...
    # Find the log entry from log parsing engine
    logs = get_all_logs()
    log_entry = next((log for log in logs if log.id == request.log_id), None)
    if not log_entry:
        raise HTTPException(
            status_code=404,
...
@router.post("/false-positives/{id}/delete")
async def remove_false_positive(
    ...
    if not success:
        raise HTTPException(
            status_code=404,
            detail="False positive entry not found or could not be removed.",
        )

    logger.info(f"False positive {id} removed from DB by {current_user.username}.")
    return {"message": "False positive entry successfully removed."}
